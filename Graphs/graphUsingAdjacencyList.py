...
class Graph:
    ...
    def addEdge(self, src, dest):
        ...
        if src == dest:
            ...
            return
        #Check if there is already an edge, checking one way is enough
        if self.checkEdgeExistence(src, dest) \
        and self.checkEdgeExistence(dest, src):
            print("\nEdge already present!\n")
            return
        #Accessing vertex head 
        srcHead = self.graph[self.vertices.index(src)]
        destHead = self.graph[self.vertices.index(dest)]
    ...
    def removeEdge(self, src, dest):
        ...
        if not self.checkEdgeExistence(src, dest) \
            and not self.checkEdgeExistence(dest, src):
            print("\nEdge already absent!\n")
            return
        ...
        #Remove the destination to source edge by traversing list of destination vertex
        temp = self.graph[self.vertices.index(dest)]
        self.disconnect(temp, src)
